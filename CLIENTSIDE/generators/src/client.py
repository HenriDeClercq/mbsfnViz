...
import argparse
import socket
#INFO: Generates random parameters and sends them to the server socket.
...
class dataSender:
    ...
    def run(self):
        while True:
            newParameters = self.generateNewRandomParameters()
            self.sendNewParametersToServer(newParameters)
            time.sleep(self.sleeptime)
    
    def generateNewRandomParameters(self):
