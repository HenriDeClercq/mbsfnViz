...
//ADDED => LINES +- 906
#include "srsenb/src/stack/mac/network_components/client_socket.cc"
#include "srsenb/src/stack/mac/network_components/client_mqtt.cc"
...
{
static int tti_counter_new = 0;
tti_counter_new = tti_counter_new+1;
          if (tti_counter_new == 10240)
          {
          tti_counter_new = 0;
          }
...
int mac::binarySearch(int arr[], int left, int right, int x) {
...
static int sf_allocs[22]={1,3,7,15,31,63,15,31,63,15,31,63,15,31,63,15,31,63,15,31,63,63};

int output = binarySearch(queue_ranges, 0, 22, 0.99*total_bytes_to_tx);  //select new SF period and SF alloc using binary search

mbsfn_period=sf_periods[output-1];

mbsfn_sfalloc=sf_allocs[output-1];

yy =100*64*mbsfn_sfalloc_new/(640*mbsfn_period);
